...
Foam::tmp < Foam::surfaceScalarField >
  Foam::multiphaseMixtureThermo::surfaceTensionForce() const {
    ...
    // const volScalarField & sigmaT = mesh_.lookupObject < volScalarField > ("sigmaT"); //sigma as func of T
    const volScalarField & T = mesh_.lookupObject < volScalarField > ("T"); //temperature
    // const volScalarField & rho = mesh_.lookupObject < volScalarField > ("rho"); //temperature

    surfaceScalarField & stf = tstf.ref();
    stf.setOriented();

    //loop through all N phases.
    forAllConstIters(phases_, phase1)
    {
      ...
      for (++phase2; phase2 != phases_.cend(); ++phase2)
      {
        ...
        //for each phase pair, look up the dsigmaT value.
        auto dSigmaT = dSigmaT_.cfind(interfacePair(alpha1, alpha2));

        if (!dSigmaT.found())
            {
                FatalErrorInFunction
                    << "Cannot find interface " << interfacePair(alpha1, alpha2)
                    << " in list of dSigmaT values"
                    << exit(FatalError);
            }
        auto plusB = plusB_.cfind(interfacePair(alpha1, alpha2));

        if (!plusB.found())
            {
                FatalErrorInFunction
                    << "Cannot find interface " << interfacePair(alpha1, alpha2)
                    << " in list of plusB values"
                    << exit(FatalError);
            }

        //calculate surfaceTension coefficient based on T at each update of alpha, moved from main
        tmp<volScalarField> sigmaT
            (
            T*dimensionedScalar("dsigmaT_", dimensionSet(1,0,-2,-1,0,0,0), *dSigmaT) +
                            dimensionedScalar("plusB", dimensionSet(1,0,-2,0,0,0,0), *plusB)
            );
        ...
        volVectorField nHatVector( //surfaceVector is also an option
          n12 / (mag(n12) +
              dimensionedScalar("tinyNumber", dimensionSet(0, -1, 0, 0, 0, 0, 0), 0.000000001)) //ROBUSTIFY
            // avoids divide by zero by adding a small but significant value
            // if mag(gradAlpha) is zero, so is the numerator. zero divide by tinyNumber is still zero.
          );
          
        nHatVector.setOriented(); //logical flag, later used in an oriented field so it needs to be consistent
        //-0.00010626 pentane
        // dsigmaT.(Tgrad - nhat.(nhat.gradT)).mag(grad(alpha))
        surfaceScalarField Tangent
          (
            fvc::interpolate(mag(n12))*
            dimensionedScalar("dSigmaT", dimensionSet(1, 0, -2, -1, 0, 0, 0), *dSigmaT) *
            (
                (
                    fvc::interpolate(fvc::grad(T)) - fvc::interpolate((nHatVector & fvc::grad(T)) * (nHatVector)) //distrubuted negative sign from leading coeff
                )// surfaceVectorField
              & mesh_.Sf()/mesh_.magSf() 
            )//dot operator, &, converts a surfaceVector to a surfaceScalar
              //here it projects the surfaceForce with the surface unit Normal
          ); //end field

        Tangent.setOriented();
        /* ensures TempTangent is a field ON the faces, not the centroids
           this is needed because the calculation above is not using field operations (e.g. snGrad) */

        surfaceScalarField sigma_normal(
          fvc::interpolate(sigmaT) * //sigma(T) coefficient, scalar
          //F = sigma.K.n   + dsigma.(Tgrad - nhat.(nhat.gradT)).mag(grad(alpha))
          (fvc::interpolate(K(alpha1, alpha2)) * //curvature, type volScalarField
            (fvc::interpolate(alpha2) * fvc::snGrad(alpha1) //surface n1 vector not nhat1
              -
              fvc::interpolate(alpha1) * fvc::snGrad(alpha2) //surface n2 vector not nhat2
            ) // when the outer multiplication of grad(alpha) is done, nhat is converted back to n on the normal component.
            // the idea is "might as well save a step and use n12 here"
          )
        );
        sigma_normal.setOriented();

        // surfaceScalarField scaleFactor( fvc::interpolate( 2*rho*(alpha1 +alpha2)/(alpha1*rho +alpha2*rho) ) );
        //relaxation factor used by Brackbill 1992
        //this biases the force to the denser phase and reduces spurious velocity in the lighter phase
        //density ratio is ~600
        //caused velocity divergence when tested.

        stf += (sigma_normal + Tangent); //total surface tension force

      } //end of interface pairs loop
    } //end of phase1 loop
    return tstf;
  } //end of surfaceTension function
...
) const
{
    ...
    // surfaceVector nHatv & mesh_.Sf() = volScalarField K
    //fvc::interpolate(K) = surfaceScalarField K
...
)
{
    ...
    int phasei = 0;
    for (phaseModel& alpha : phases_) //loop through phases
    {
        alphaPhiCorrs.set
        (
            phasei,
